...
alexRenew = pd.read_csv('../AlexRenew/censored_replaced2022.csv') ##influent quality and precipitation
alexRenew['date'] = pd.to_datetime(alexRenew['date'])
houston = pd.read_csv('../NE Houston/nehouston_complete.csv') #influent quality and flow
houston['date']= pd.to_datetime(houston['date'])
fig, axes = plt.subplots(1, 2)
sns.boxplot(x='season', y='rainfall_inches', orient='v', data=alexRenew, ax=axes[0])
sns.boxplot(x='season', y='rainfall_inches', orient='v',data=houston, ax=axes[1])
plt.show()
plt.savefig('boxplots_rainfall.png',dpi = 1200)

boxplot = sns.boxplot(x='season', y='flow_mgd', data=pd.melt(alexRenew))
boxplot.set_xlabel("meteorological seasons", fontsize=14)
boxplot.set_ylabel("Flow in MGD", fontsize=14)
plt.show()
category = houston[['season']] #, 'season_Spring',
#'season_Summer', 'season_Winter']]
for x in category:
    model = ols('flow_mgd' + '~' + x, data = houston).fit() #Oridnary least square method
    result_anova = smf.stats.anova_lm(model)
    with open('houstoncateg.txt', 'a') as output:
                print('flow_mgd', result_anova, file = output)
                
                
import matplotlib.gridspec as gridspec  
testingPred = pd.read_csv('../NE Houston/houstonpredictions.csv')
testingPred = pd.read_csv('../AlexRenew/alexRenew predictions.csv')
testingPred['date'] = pd.to_datetime(testingPred['date'])
# plt.subplots(figsize=(4, 4))

plt.subplots(figsize=(4, 4))
G = gridspec.GridSpec(1, 1)

fig, ax = plt.subplots(2,1, figsize=(4,4), sharex = True,gridspec_kw={'height_ratios': [1.8, 1.2]})
ax[0].plot('date', 'flow_mgd', label ='measured flow', linestyle ='solid',data = testingPred, color='k')
ax[0].plot('date', 'yhat', ls = 'solid', label ='predicted flow (orig. train)', data = testingPred, color = 'orange')
ax[0].plot('date', 'yhat_bal', ls = 'solid', label ='predicted flow (resampled train)',data = testingPred, color = 'green')
ax[0].plot('date', 'wet_weather', ls = 'dashed', label ='wet weather',data = testingPred, color = 'red')
# ax.fill_between(testingPred.date, 'yhat_lower', 'yhat_upper', alpha = 0.3, color= 'tab:orange', data = testingPred)

ax[1].bar(testingPred.date, testingPred.rainfall_inches, color='tab:blue',alpha=0.8)


# ax[0].set_xlim([date.date(2020, 8, 8), date.date(2020,12, 31)])
# ax[0].set_xlim([date.date(2020, 10, 8), date.date(2020,12, 31)])
# ax[0].set_xlim([date.date(2019, 3, 10), date.date(2019,6, 30)])
ax[0].set_xlim([date.date(2019, 5, 1), date.date(2019,6, 10)])
date_form = mdates.DateFormatter('%Y-%m-%d')
ax[0].xaxis.set_major_formatter(date_form)
week = mdates.WeekdayLocator(interval =2)
day = mdates.DayLocator()
ax[0].xaxis.set_major_locator(week)
fig.tight_layout()
# plt.show()
ax[0].legend(prop={"size":5}, loc = 'best')
ax[0].legend(prop={"size":5}, loc = 'best')
plt.savefig('MeasuredvsPredictedFlowFeb2023_for_abstractH.png',dpi = 1200)



#########forecasting model plots#########

testingPred = pd.read_csv('../AlexRenew/forecasting.csv')
testingPred = pd.read_csv('../NE Houston/forecasting.csv')
testingPred['date'] = pd.to_datetime(testingPred['date'])
fig, ax = plt.subplots(2,1, figsize=(4,4), sharex = True,gridspec_kw={'height_ratios': [1.8, 1.2]})
ax[0].plot('date', 'pred', label =None, linestyle ='solid',color = 'orange',data = testingPred)
ax[0].plot('date', 'flow', label =None, linestyle ='solid',color = 'k',data = testingPred)
ax[0].fill_between('date', 'lower_bound', 'upper_bound', color='orange', alpha=0.1, data = testingPred)

ax[1].bar(testingPred.date, testingPred.rainfall_inches,
       color='tab:blue', alpha = 0.8, label = 'Rainfall')

date_form = mdates.DateFormatter('%Y-%m-%d')
ax[0].xaxis.set_major_formatter(date_form)
week = mdates.WeekdayLocator(interval =2)
day = mdates.DayLocator()
ax[0].xaxis.set_major_locator(day)
fig.tight_layout()
# ax[0].legend(prop={"size":10}, loc = 2, frameon=False)
# ax[0].legend(prop={"size":10}, loc = 'best', frameon=False)
plt.savefig('ForecastingFlow_alex_modified_Feb27.png',dpi = 1200)
...
houston['date']= pd.to_datetime(houston['date'])
test_data = 113
train_houston = houston[:-test_data]
valid_houston=train_houston.iloc[-50:]
train_houston= pd.concat([train_houston, valid_houston]).drop_duplicates(keep=False).copy()
test_houston=houston[-test_data:]

train_labels = train_houston['flow_mgd']
valid_labels = valid_houston['flow_mgd']
test_labels = test_houston['flow_mgd']

houstonBal = pd.read_csv('../NE Houston/houstonsyntheticbal.csv')
trainBal_labels = houstonBal['flow_mgd']


alexRenew = pd.read_csv('../AlexRenew/censored_replaced2022.csv') ##influent quality and precipitation
alexRenew['date'] = pd.to_datetime(alexRenew['date'])
test_data = 146
train_alexRenew = alexRenew[:-test_data]
valid_alexRenew=train_alexRenew.iloc[-100:]
train_alexRenew= pd.concat([train_alexRenew, valid_alexRenew]).drop_duplicates(keep=False).copy()
test_alexRenew=alexRenew[-test_data:]

train_labels = train_alexRenew['flow_mgd']
valid_labels = valid_alexRenew['flow_mgd']
test_labels = test_alexRenew['flow_mgd']

alexrenewBal = pd.read_csv('../AlexRenew/alexRenewsyntheticbal.csv')
trainBal_labels = alexrenewBal['flow_mgd']
...
trainNE_labels = train_houston['flow_mgd']
valid_labels = valid_houston['flow_mgd']
test_labels = test_houston['flow_mgd']

houstonBal = pd.read_csv('../NE Houston/houstonsyntheticbal.csv')
houstonBal = pd.get_dummies(houstonBal, prefix=None)
trainBalNE_labels = houstonBal['flow_mgd']
# rcParams['font.family'] = 'serif'
# my_cmap = cm.get_cmap('seismic')
fig, ax = plt.subplots(1,2, figsize=(10,4))
sns.kdeplot(train_labels, color='orange', label ='Original Training Set',ax = ax[0])
sns.kdeplot(trainBal_labels,label = "Resampled Training Set", color='green', ax = ax[0])
sns.kdeplot(trainNE_labels, color='orange', label ='Original Training Set', ax = ax[1], legend=False)
sns.kdeplot(trainBalNE_labels,label = "Resampled Training Set", color='green', ax = ax[1], legend=False)
# plt.ylabel('Kernel Density Estimate', fontsize=10)
# plt.show()
plt.savefig('alexHoustonDensityPlotsFeb27.png', dpi=1200,bbox_inches ='tight')



####data count####################
# alexRenew = pd.read_excel('../AlexRenew/censored_replaced2022.xlsx')
neHouston = pd.read_excel('../NE Houston/nehouston_complete.xlsx')
my_cmap = cm.get_cmap('seismic')
plt.figure(figsize=(8,8))
hi = neHouston.groupby(["event", "season"]).size()
alexRenew.groupby(["event", "season"]).size().plot.bar(color= my_cmap(hi))
plt.savefig('AlexRenew Event and Season Data.png',dpi = 1200, bbox_inches ='tight')    